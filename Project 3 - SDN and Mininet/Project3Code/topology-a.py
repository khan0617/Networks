#!/usr/bin/env python

from mininet.cli import CLI
from mininet.net import Mininet
from mininet.link import TCLink
from mininet.topo import Topo
from mininet.log import setLogLevel
from mininet.node import RemoteController

class AssignmentNetworks(Topo):
    def __init__(self, **opts):
        Topo.__init__(self, **opts)

		# define all the hosts
        h1 = self.addHost('h1')
        h2 = self.addHost('h2')
        h3 = self.addHost('h3')
        h4 = self.addHost('h4')
        h5 = self.addHost('h5')
        h6 = self.addHost('h6')
        h7 = self.addHost('h7')
        h8 = self.addHost('h8')
        h9 = self.addHost('h9')

		# define each switch
        s1 = self.addSwitch('s1')
        s2 = self.addSwitch('s2')
        s3 = self.addSwitch('s3')
        s4 = self.addSwitch('s4')
        s5 = self.addSwitch('s5')

		# links between each host and the switches
        self.addLink(h1, s1)
        self.addLink(h2, s1)
        self.addLink(h3, s2)
        self.addLink(h4, s3)
        self.addLink(h5, s4)
        self.addLink(h6, s4)
        self.addLink(h7, s5)
        self.addLink(h8, s5)
        self.addLink(h9, s5)

		# links between switches, with bandwidth (Mbps) and delay (ms)
        self.addLink(s1, s2, bw=20, delay='40ms')
        self.addLink(s2, s3, bw=40, delay='10ms')
        self.addLink(s3, s4, bw=30, delay='20ms')
        self.addLink(s3, s5, bw=20, delay='30ms')
        
def run_iperf(net, src, dst, duration):
	print("Running iperf between {} and {} for {} seconds...".format(src, dst, duration))
	src, dst = net.get(src, dst)
	output = net.iperf((src, dst), l4Type='TCP', seconds=duration)
	# print(f"Results for {src} -> {dst}:\n{output}\n")
	print("Results for {} -> {}:{}\n".format(src, dst, output))
                
if __name__ == '__main__':
	setLogLevel( 'info' )

	# Create data network
	topo = AssignmentNetworks()
	net = Mininet(topo=topo, link=TCLink, autoSetMacs=True,autoStaticArp=True)
        
    # uncomment below line to use custom controller w/ ethernet-learning.py
	#net = Mininet(controller=RemoteController,topo=topo, link=TCLink, autoSetMacs=True, autoStaticArp=True)

	# Run network
	net.start()
	run_iperf(net, 'h1', 'h3', 20)  # s1 -> s2 (Link L1)
	run_iperf(net, 'h3', 'h4', 20)  # s2 -> s3 (Link L2)
	run_iperf(net, 'h4', 'h6', 20)  # s3 -> s4 (Link L3)
	run_iperf(net, 'h4', 'h7', 20)  # s3 -> s5 (Link L4)
	CLI( net )
	net.stop()

